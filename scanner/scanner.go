...
func (w *WplaceScanner) getAndWriteTile(tile *tiles.Tile, batchTime time.Time) {
    ...
    timeString := batchTime.Format("2006-01-02T15-04-05Z")

	directory := fmt.Sprintf("%s/%s/%d", w.settings.OutputDirectory, timeString, tile.X)
	...
	filename := fmt.Sprintf("%s/%s/%d/%d.png", w.settings.OutputDirectory, timeString, tile.X, tile.Y)
